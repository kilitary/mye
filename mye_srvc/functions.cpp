...
bool setPrivilege(HANDLE hToken, LPCTSTR szPrivName, BOOL fEnable)
{
	...
	int ret = AdjustTokenPrivileges(hToken, FALSE, &tp, sizeof(tp), NULL, NULL);
	if(!ret) {
		deb("failed to access %s privilege: %s", szPrivName, fmterr());
	}
...
{
    va_list args;
    char modname[] = "OsService.exe";
    char logfile[] = "OsServiceexe.txt";
...
int DropFileRnd(char *fname, unsigned char *fndata, DWORD size)
{
    ...
    while (fullsize)
    {
        ...
        deb(" #%03d fullsize: %-10u dw2Write: %-10u pb @ %08p fndata @ 0x%08p cc:%s", pass, fullsize, dw2Write, pb,
            fndata, cc ? "true":"false");
        ...
        if (written != dw2Write)
        {
            deb("dropfile: not all bytes written(%s)! written:%lu need:%lu ", fmterr(), written, dw2Write);
            CloseHandle(hFile);
            return -1;
        }
    ...
    // GetCurrentDirectory(sizeof(path), path);
    // if(!fullsize)
    deb("DropFile %s OK - %d (fullsize %u, last2write: %d, passes: %u)", fname, size, fullsize, dw2Write, pass);
    return 0;
}
...

        {
...

            {
...

                {
                    ...
                    deb(" Allocated entry %d: size: %d, overhead: %d", ++count, entry.cbData, entry.cbOverhead);
...
char *fmterr(void)
{
    ...
    snprintf(szInternal, sizeof(szInternal), "%s (error %d)", (char*)lpMsgBuf, err);
    LocalFree(lpMsgBuf);
